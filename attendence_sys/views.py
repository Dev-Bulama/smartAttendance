...
from .forms import *
from .models import Student, Attendence, Course, Session, Grade, CourseGrade, Faculty
...
def student_grades(request):
    """View all grades for logged-in student"""
    try:
        student = request.user.student
    except:
        messages.error(request, 'You do not have student access.')
    ...
    my_grades = Grade.objects.filter(student=student).select_related('course', 'lecturer').order_by('-date_recorded')
    ...
    if selected_year:
        my_grades = my_grades.filter(academic_year=selected_year)
    ...
    academic_years = Grade.objects.filter(student=student).values_list('academic_year', flat=True).distinct()
    
    # Organize grades by course
    courses_dict = {}
    for grade in my_grades:
        course_key = f"{grade.course.course_code}_{grade.semester}_{grade.academic_year}"
        ...
        courses_dict[course_key]['grades'].append(grade)
        courses_dict[course_key]['total_percentage'] += float(grade.percentage)
        courses_dict[course_key]['count'] += 1
    
    # Calculate averages for each course
    for course_data in courses_dict.values():
        if course_data['count'] > 0:
            course_data['average'] = round(course_data['total_percentage'] / course_data['count'], 2)
            
            # Determine letter grade based on average
            avg = course_data['average']
            if avg >= 90:
                course_data['letter_grade'] = 'A+'
            elif avg >= 85:
                course_data['letter_grade'] = 'A'
            elif avg >= 80:
                course_data['letter_grade'] = 'B+'
            elif avg >= 75:
                course_data['letter_grade'] = 'B'
            elif avg >= 70:
                course_data['letter_grade'] = 'C+'
            elif avg >= 65:
                course_data['letter_grade'] = 'C'
            elif avg >= 60:
                course_data['letter_grade'] = 'D'
            else:
                course_data['letter_grade'] = 'F'
        else:
            course_data['average'] = 0
            course_data['letter_grade'] = 'N/A'
    
    # Calculate overall average
    if my_grades.exists():
        total_percentage = sum(float(g.percentage) for g in my_grades)
        overall_average = round(total_percentage / my_grades.count(), 2)
    else:
        overall_average = 0
    ...
    return render(request, 'attendence_sys/student_grades.html', context)
...
def student_transcript(request):
    ...
    # Get all grades for this student, organized by semester and year
    all_grades = Grade.objects.filter(student=student).select_related('course').order_by('academic_year', 'semester', 'course__course_code')
    
    # Organize by semester and year
    transcript_data = {}
    
    for grade in all_grades:
        key = f"{grade.academic_year}_{grade.semester}"
        ...
        if course_code not in transcript_data[key]['courses']:
            transcript_data[key]['courses'][course_code] = {
                'course': grade.course,
                'grades': [],
                'total_percentage': 0,
                'count': 0
        ...
        transcript_data[key]['courses'][course_code]['grades'].append(grade)
        transcript_data[key]['courses'][course_code]['total_percentage'] += float(grade.percentage)
        transcript_data[key]['courses'][course_code]['count'] += 1
    
    # Calculate course averages for each semester
    for semester_data in transcript_data.values():
        ...
        for course_data in semester_data['courses'].values():
            if course_data['count'] > 0:
                course_avg = course_data['total_percentage'] / course_data['count']
                course_data['average'] = round(course_avg, 2)
                ...
                semester_total += course_avg
                semester_count += 1
        
        # Calculate semester average
        semester_data['semester_average'] = round(semester_total / semester_count, 2) if semester_count > 0 else 0
    
    # Calculate cumulative average
    if all_grades.exists():
        total_percentage = sum(float(g.percentage) for g in all_grades)
        cumulative_average = round(total_percentage / all_grades.count(), 2)
    else:
    ...
    return render(request, 'attendence_sys/student_transcript.html', context)
...
@login_required(login_url='login')
def takeAttendence(request):
    """Take attendance using browser-based face recognition"""
    try:
        faculty = request.user.faculty
    except:
        messages.error(request, 'You do not have lecturer access.')
    ...
    if request.method == 'POST':
        # Check if this is the initial form or the face recognition result
        if 'verified_students' in request.POST:
            # Process face recognition results
            branch = request.POST.get('branch')
            year = request.POST.get('year')
            section = request.POST.get('section')
            period = request.POST.get('period')
            verified_students = request.POST.get('verified_students', '')
            ...
            if Attendence.objects.filter(
                ...
                messages.error(request, "Attendance already recorded for this period.")
                return redirect('lecturer_dashboard')
            
            # Get all students in the class
            students = Student.objects.filter(branch=branch, year=year, section=section)
            
            # Parse verified student IDs
            verified_ids = [s.strip() for s in verified_students.split(',') if s.strip()]
            
            # Mark attendance for all students
            for student in students:
                status = 'Present' if student.registration_id in verified_ids else 'Absent'
            ...
            messages.success(request, f"Attendance marked successfully! {len(verified_ids)} students present.")
            ...
            # Initial form submission - show face recognition interface
            details = {
                'branch': request.POST.get('branch'),
                'year': request.POST.get('year'),
                'section': request.POST.get('section'),
            ...
            students = Student.objects.filter(
                ...
                year=details['year'],
                section=details['section']
            ).filter(profile_pic__isnull=False).exclude(profile_pic='')
            
            if not students.exists():
                messages.warning(request, 'No students with profile pictures found in this class.')
                return redirect('lecturer_dashboard')
            ...
            return render(request, 'attendence_sys/take_attendance_face.html', context)
